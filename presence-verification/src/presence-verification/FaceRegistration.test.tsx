...
describe("FaceRegistration", () => {
  it("explains privacy first, then takes 5 photos and uploads them together", async () => {
    const { register, track, onRegistered, user } = setup();
    expect(screen.getByText(/never your photos/i)).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Start Camera" }));

    for (const n of [1, 2, 3, 4, 5]) {
      expect(await screen.findByRole("heading", { name: `Photo ${n} of 5` })).toBeInTheDocument();
      await capture(user);
    }
  ...
  it("shows which photo was rejected and lets the intern start again", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Start Camera" }));
    for (let i = 0; i < 5; i++) {
      await screen.findByRole("heading", { name: `Photo ${i + 1} of 5` });
      await capture(user);
    }
